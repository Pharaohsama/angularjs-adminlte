(function () {
    ...
    function configFn($stateProvider) {
        $stateProvider.state('app.editMovie', {
            url: '/editMovie/:id',
            views: {
                'content@app': {
                    templateUrl: 'app/pages/movie/movieEdit/movieEdit.html',
                    controller: ControllerFn,
                    controllerAs: 'vmMovieEdit'
                }
            }
        });
    }

    // ControllerFn.$inject = ["UserService", "$location", "$scope", "$http", "$stateParams"];
     ...
     * @constructor
     */
    function ControllerFn(UserService, $location, $scope, $http, $stateParams) {
        var vm = this;
        $scope.file = null;
        ...
        console.log(movieId);
        $scope.getData = () => {
            $http.get("http://localhost:8080/api/movies/" +movieId)
        ...
        $scope.postData = function (title, description, duration, release, na, dir, actors, genres, pos) {
            ...
            $http.patch("http://localhost:8080/api/movies/"+movieId, dataForm, {
